...
namespace RealEstateAgency.Controllers.Estate
{
    public partial class PropertyController
    {
        ...
        public async Task<ActionResult<WebSiteEstateDto>>
            GetWebAppEstate(int propertyId,
                [FromQuery(Name = "referral")] string referralCode,
                [FromQuery(Name = "media")] string socialMedia, CancellationToken cancellationToken)
        {
        ...
        private async Task SavePropertyView(string referralCode, string socialMedia
            , CancellationToken cancellationToken)
        {
            var network = _networkProvider[socialMedia];
            if (network is null) return;
            var shared = await _sharedPropertyService
                .GetAsync(s => s.ReferralCodeAndSharingNetworkId
                               == referralCode && s.SocialNetworkId == network.Id, cancellationToken);
            if (shared is null) return;
            await _sharedPropertyClickService.CreateAsync(new SharedPropertyClick
            {
                MetaData = "{}",
